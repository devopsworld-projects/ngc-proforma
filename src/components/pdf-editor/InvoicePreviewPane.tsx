      ...
      <table className="w-full text-xs" style={{ borderColor: tableBorderColor }}>
        <thead>
          <tr style={{ backgroundColor: settings.table_header_bg, color: settings.table_header_text }}>
            ...
            {showGst && <th className={`${tableRowPaddingClass} text-right font-semibold uppercase text-[10px]`}>GST Amt (18%)</th>}
            {!showGst && <th className={`${tableRowPaddingClass} text-right font-semibold uppercase text-[10px]`}>Unit Price</th>}
        ...
        <tbody style={{ color: settings.table_text_color }}>
          {sampleItems.map((item) => (
            <tr key={item.id} className={tableBorder} style={{ borderColor: tableBorderColor }}>
              ...
              {!showGst && <td className={`${tableRowPaddingClass} text-right font-mono`}>₹{item.rate.toLocaleString()}</td>}
